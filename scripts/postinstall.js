...
// Custom VDXF keys (from any namespace) are not in the library's
...
if (existsSync(vdxfPath)) {
  ...
  const fix1 = `// verus-tx-offline patch: treat unknown VDXF keys as DataDescriptor (opaque passthrough)
  ...
  const fix1b = `// verus-tx-offline patch: treat unknown VDXF keys as DataDescriptor (opaque passthrough)
                const descr = new DataDescriptor_1.DataDescriptor(value);
  ...
  const fix3 = `// verus-tx-offline patch: treat unknown VDXF keys as DataDescriptor (opaque passthrough)
